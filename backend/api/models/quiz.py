...
class Question(BaseModel):
    ...
    # --- Misconception tagging (DB/bank payload contract) ---
    misconception_info: Optional[List[Dict[str, Any]]] = Field(
        None,
        description=(
            "Optional misconception annotations keyed by distractor value. "
            "Expected shape: [{\"type\": <MisconceptionType or str>, \"value\": <option str>, ...}]"
        ),
    )

    # Optional passthrough for DB-served questions that wrap a stored payload dict.
    payload: Optional[Dict[str, Any]] = Field(
        None,
        description="Raw stored payload for bank/served questions (used by session adapter).",
    )
    meta: Optional[Dict[str, Any]] = Field(
        None,
        description="Additional metadata wrapper for questions (legacy compatibility).",
    )
